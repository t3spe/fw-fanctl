...
# Install fw-fanctl daemon to system paths:
#   /usr/local/lib/fw-fanctl/fw_fanctl.py
#   /etc/fw-fanctl/config.json
#   /etc/systemd/system/fw-fanctl.service
#
# Usage:
...
LIB_DIR="/usr/local/lib/fw-fanctl"
CONF_DIR="/etc/fw-fanctl"
SERVICE="/etc/systemd/system/fw-fanctl.service"
...
# --- Uninstall ---
if [ "${1:-}" = "--uninstall" ]; then
    echo "Uninstalling fw-fanctl..."
    sudo systemctl stop fw-fanctl 2>/dev/null || true
    sudo systemctl disable fw-fanctl 2>/dev/null || true
    sudo rm -f "$SERVICE"
    # Guard: only rm -rf if the path looks right
    if [ -d "$LIB_DIR" ] && [ "$LIB_DIR" = "/usr/local/lib/fw-fanctl" ]; then
        sudo rm -rf "$LIB_DIR"
    fi
    sudo systemctl daemon-reload
    echo "Removed service and daemon."
    echo "Config left in $CONF_DIR (remove manually if desired)."
    echo "Sensor logs left in /var/log/fw-fanctl (remove manually if desired)."
    echo "Done."
    exit 0
fi
...
if ! command -v systemctl >/dev/null 2>&1; then
    ...
    echo "  You can still run the daemon manually: sudo python3 fw_fanctl.py"
    exit 1
fi
...
for f in fw_fanctl.py config.json fw-fanctl.service.template; do
...
if ! python3 -c "import py_compile, os; py_compile.compile(os.path.join(os.environ['_FANCTL_DIR'], 'fw_fanctl.py'), doraise=True)" 2>/dev/null; then
    echo "error: fw_fanctl.py has syntax errors"
    exit 1
fi

# Preflight: verify config is valid JSON
if ! python3 -c "import json, os; json.load(open(os.path.join(os.environ['_FANCTL_DIR'], 'config.json')))" 2>/dev/null; then
...
import json, sys, os
d = os.environ['_FANCTL_DIR']
sys.path.insert(0, d)
from fw_fanctl import validate_config
...
WAS_RUNNING=false
if systemctl is-active --quiet fw-fanctl 2>/dev/null; then
    WAS_RUNNING=true
    echo "Stopping existing fw-fanctl service..."
    sudo systemctl stop fw-fanctl
fi

# Create sensor log directory
sudo mkdir -p /var/log/fw-fanctl

# Install daemon
echo "Installing daemon to $LIB_DIR..."
sudo mkdir -p "$LIB_DIR"
sudo cp "$SCRIPT_DIR/fw_fanctl.py" "$LIB_DIR/fw_fanctl.py.new"
sudo chmod 755 "$LIB_DIR/fw_fanctl.py.new"
sudo mv "$LIB_DIR/fw_fanctl.py.new" "$LIB_DIR/fw_fanctl.py"
...
if [ -f "$CONF_DIR/config.json" ]; then
    if diff -q "$SCRIPT_DIR/config.json" "$CONF_DIR/config.json" >/dev/null 2>&1; then
        ...
        export _FANCTL_CONF_DIR="$CONF_DIR"
        if ! existing_err=$(python3 -c "
import json, sys, os
d = os.environ['_FANCTL_DIR']
sys.path.insert(0, d)
from fw_fanctl import validate_config
with open(os.path.join(os.environ['_FANCTL_CONF_DIR'], 'config.json')) as f:
    validate_config(json.load(f))
" 2>&1); then
...
sed -e "s|@LIB_DIR@|$LIB_DIR|g" \
    ...
    -e "s|@ECTOOL@|$ECTOOL|g" \
    -e '/^# Substituted by/,/^$/d' \
    "$SCRIPT_DIR/fw-fanctl.service.template" | sudo tee "$SERVICE" >/dev/null

sudo systemctl daemon-reload

# Restart if it was running before (upgrade path)
if [ "$WAS_RUNNING" = true ]; then
    echo "Restarting fw-fanctl service..."
    sudo systemctl start fw-fanctl
fi
...
echo ""
echo "Commands:"
echo "  sudo systemctl enable --now fw-fanctl   # enable and start"
echo "  sudo bash check.sh                       # verify installation"
echo "  sudo systemctl status fw-fanctl          # check status"
echo "  journalctl -u fw-fanctl -f               # follow logs"
echo ""
echo "Uninstall:"
