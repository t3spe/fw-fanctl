...
"""Tests for fw_fanctl: SensorLogger, controllers, Hardware, run().

Run without root:  python3 tests/test_fw_fanctl.py
...
check("default path", sl.path == "/var/log/fw-fanctl/sensor-log.json")
...
check("--version prints version", "fw-fanctl" in result.stdout,
...
with tempfile.TemporaryDirectory() as td:
    ...
    # Patch Path at the fw_fanctl import site so the real functions
    ...
    hw_test = Hardware()
    with _patch("fw_fanctl.Path", side_effect=_fake_path):
