...
# Verify fw-fanctl installation and runtime state.
...
if [ -f "$SCRIPT_DIR/fw_fanctl.py" ]; then
    PYTHON_IMPORT_DIR="$SCRIPT_DIR"
elif [ -f "$LIB_DIR/fw_fanctl.py" ]; then
    PYTHON_IMPORT_DIR="$LIB_DIR"
else
    PYTHON_IMPORT_DIR=""
fi
export _FANCTL_IMPORT_DIR="$PYTHON_IMPORT_DIR"
...
if [ -f "$LIB_DIR/fw_fanctl.py" ]; then ok "daemon"; else fail "daemon not at $LIB_DIR/fw_fanctl.py"; fi
...
if [ -d "/var/log/fw-fanctl" ]; then
    if [ -w "/var/log/fw-fanctl" ]; then ok "log dir"; else warn "log dir not writable"; fi
else
    warn "log dir /var/log/fw-fanctl missing (install.sh creates it)"
fi

# --- Files match repo ---
echo "Repo sync:"
if [ -f "$SCRIPT_DIR/fw_fanctl.py" ]; then
    if [ -f "$LIB_DIR/fw_fanctl.py" ]; then
        if diff -q "$SCRIPT_DIR/fw_fanctl.py" "$LIB_DIR/fw_fanctl.py" >/dev/null 2>&1; then
...
echo "Config:"
if [ -f "$CONF_DIR/config.json" ]; then
    if python3 -c "import json, os; json.load(open(os.path.join(os.environ['_FANCTL_CONF_DIR'], 'config.json')))" 2>/dev/null; then
        ok "valid JSON"
    else
        fail "invalid JSON"
    fi
...
import sys, json, os
sys.path.insert(0, os.environ['_FANCTL_IMPORT_DIR'])
from fw_fanctl import validate_config
validate_config(json.load(open(os.path.join(os.environ['_FANCTL_CONF_DIR'], 'config.json'))))
" 2>&1)
        validate_rc=$?
        ...
        info "skipping validation (fw_fanctl.py not found)"
    fi
fi
...
if [ "$enabled_state" = "enabled" ] || [ "$enabled_state" = "enabled-runtime" ]; then
    ...
    warn "not enabled (run: sudo systemctl enable fw-fanctl)"
fi
if systemctl is-active --quiet "$SERVICE" 2>/dev/null; then
    ...
    warn "not running (run: sudo systemctl enable --now fw-fanctl)"
fi
...
if [ -n "$PYTHON_IMPORT_DIR" ]; then
    # coretemp
    ct_path=$(python3 -c "
import sys, os; sys.path.insert(0, os.environ['_FANCTL_IMPORT_DIR'])
from fw_fanctl import Hardware
...
" 2>/dev/null)
    ...
    info "skipping sensor discovery (fw_fanctl.py not found)"
fi
